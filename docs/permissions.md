...
We can use the `#[ACL\HasPermissions]` attribute to define the permissions relation.
...
When adding the `LaravelDoctrine\ACL\Permissions\HasPermissions` trait you will get a `hasPermissionTo` method.
