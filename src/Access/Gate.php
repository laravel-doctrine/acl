...
/**
 * Copy of Laravel's Gate contract implementation with changes to support Doctrine proxy classes
 * @author Gerben van Eck
 */

namespace LaravelDoctrine\ACL\Access;
...
class Gate implements GateContract
{
    ...
    protected function firstArgumentCorrespondsToPolicy(array $arguments)
    {
        ...
        if (is_object($arguments[0])) {
            $parents = class_parents($arguments[0]);
            $parentclass = null;
            if(empty($parents)) {
                return false;
            } else {
                $parentclass = array_shift($parents);
            }
            return isset($this->policies[$parentclass]);
        }
